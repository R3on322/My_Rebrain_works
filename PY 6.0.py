...
for i in range(len(list)):
        dictionary = dict()
        dictionary['date'] = ' '.join(list[i][:2])
        dictionary['time'] = ''.join(list[i][2])
...
for i in range(len(list_dict)):
    if list_dict[i]["date"] == 'May 20':
        list_only_20.append(list_dict[i])

with open('file_6.txt', 'w+', encoding='cp1251') as file:
    file.write(str(list_only_20))
#3.0

    file.seek(0)
    exec('list_20_may = ' + file.read())
    print(list_only_20[0]['time'])